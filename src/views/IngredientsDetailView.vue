//src/views/IngredientsDetailView.vue
<script setup lang="ts">
import { onBeforeRouteUpdate, useRoute, useRouter } from 'vue-router'
import { onMounted, ref } from 'vue'
import { getRecommendedProducts, searchIngredientsByName } from '@/api'
import { ScrollArea } from '@/components/ui/scroll-area'

const route = useRoute()
const router = useRouter()

const ingredientDetail = ref({
  ingredientName: '',
  description: '',
} as any)

const recommendedProducts = ref([] as any)

onBeforeRouteUpdate((to, from, next) => {
  fetchData(to.params.ingredientName as string)
  next()
})

const fetchIngredientDetail = async (ingredientName: string) => {
  try {
    const decodedName = decodeURIComponent(ingredientName)
    const response = await searchIngredientsByName({
      substanceDetected: decodedName,
    })
    ingredientDetail.value = {
      ingredientName: decodedName,
      description: response.data || 'No description available for this ingredient.',
    }
  } catch (error) {
...
const fetchData = async (ingredientName: string) => {
  loading.value = true
  try {
    await Promise.all([fetchIngredientDetail(ingredientName), fetchRecommendationProducts()])
  } catch (error) {
...
onMounted(() => {
  fetchData(route.params.ingredientName as string)
})
</script>
...
<template>
  <div class="page">
    <!-- 成分名称 -->
    <h1 class="product-name">{{ ingredientDetail.ingredientName }}</h1>

    <!-- 红色警告框 -->
    <div class="alert-box">
      <scroll-area class="h-9/10 w-2/3">
        <p class="reasoning-text">{{ ingredientDetail.description }}</p>
      </scroll-area>
